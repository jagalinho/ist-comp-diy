...
extern void init(int pub, int typ, char *id);
extern void initAtr(int pub, int typ, char *id, Node *atr);
extern void initConstAtr(int pub, int typ, char *id, Node *atr);
extern void finit(int pub, int typ, char *id, Node *body, int enter);
extern void gen_extrn();
...
%token <r> REAL
%token <s> ID STR
%token DO WHILE IF THEN FOR IN UPTO DOWNTO STEP BREAK CONTINUE GOTO
...
file:
	| file error ';'
	| file public tipo ID ';'	        { IDnew($3->value.i, $4, 0); declare($2, 0, $3, $4, 0); init($2, $3->value.i, $4); }
	| file public CONST tipo ID ';'	    { IDnew($4->value.i+5, $5, 0); declare($2, 1, $4, $5, 0); init($2, $4->value.i, $5); }
	| file public tipo ID init	        { IDnew($3->value.i, $4, 0); declare($2, 0, $3, $4, $5); initAtr($2, $3->value.i, $4, $5); }
	| file public CONST tipo ID init	{ IDnew($4->value.i+5, $5, 0); declare($2, 1, $4, $5, $6); initConstAtr($2, $4->value.i, $5, $6); }
	| file public tipo ID { enter($2, $3->value.i, $4); fargs = 8; } finit { function($2, $3, $4, $6); finit($2, $3->value.i, $4, $6, -flocal); }
	| file public VOID ID { enter($2, 4, $4); fargs = 8; } finit { function($2, intNode(VOID, 4), $4, $6); finit($2, 4, $4, $6, -flocal); }
	;
...
param: tipo ID    { $$ = binNode(PARAM, $1, strNode(ID, $2));
                    if (IDlevel() == 1) {
                        IDnew($1->value.i, $2, fargs);
                        fpar[++fpar[0]] = $1->value.i;
                        fargs += $1->value.i == 3 ? 8 : 4;
                    } else {
                        flocal -= $1->value.i == 3 ? 8 : 4;
                    	IDnew($1->value.i, $2, flocal);
                    }}
	 ;
...
expr: lv		               { $$ = uniNode(PTR, $1); $$->info = $1->info; }
	| '*' lv                   { $$ = uniNode(PTR, uniNode(PTR, $2)); 
                                    if ($2->info % 5 == 2) $$->info = 1; 
                                    else if ($2->info / 10 == 1) $$->info = $2->info % 10; 
                                    else yyerror("can dereference lvalue"); }
	| lv ATR expr              { $$ = binNode(ATR, $3, $1); 
                                    if ($$->info % 10 > 5) yyerror("constant value to assignment");
                                    if (noassign($1, $3)) yyerror("illegal assignment");
                                    $$->info = $1->info; }
	...
	| expr '=' expr            { $$ = binNode('=', $1, $3); $$->info = 1; }
	| expr XOR expr				{ $$ = binNode(XOR, $1, $3); $$->info = 1; }
	| expr '&' expr            { $$ = binNode('&', $1, $3); $$->info = intonly($1, 0); intonly($3, 0); }
	| expr '|' expr            { $$ = binNode('|', $1, $3); $$->info = intonly($1, 0); intonly($3, 0); }
	| '(' expr ')'             { $$ = $2; $$->info = $2->info; }
	| ID '(' args ')'          { $$ = binNode(CALL, strNode(ID, $1), $3); $$->info = checkargs($1, $3); }
	| ID '(' ')'               { $$ = binNode(CALL, strNode(ID, $1), nilNode(NIL)); $$->info = checkargs($1, 0); }
	;
...
void enter(int pub, int typ, char *name) {
    ...
    if (typ == 3)
        ...
        flocal = -4;
	IDpush();
	if (typ != 4) IDnew(typ, name, flocal);
}
